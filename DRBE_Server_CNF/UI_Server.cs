...
namespace DRBE_Server_CNF
{
    public class UI_Server
    {
        ...
        public bool UI_wirte_stream_connected_flag = false;
        ...
        private TcpClient Matlab_Client;
        private NetworkStream Matlab_nwStreamwrite;
        public bool Matlab_Client_inserted_flag = false;
        public void Connect_Matlab(TcpClient x, NetworkStream y)
        {
            Matlab_Client = x;
            Matlab_nwStreamwrite = y;
            Matlab_Client_inserted_flag = true;
        }
        ...
        private void Packet_receiver(byte x)
        {
            ...
            else if (Packet_receiver_index == Packet_len + 7)
            {
                ...
                {
                    if (Packet_command == 0x10 && Matlab_Client_inserted_flag)
                    {
                        Console.WriteLine("Send to Matlab: " + Packet_receiver_result.Count.ToString());
                        Matlab_nwStreamwrite.Write(Packet_receiver_result.ToArray(), 0, Packet_receiver_result.Count);
                        Matlab_nwStreamwrite.Flush();
                    }
